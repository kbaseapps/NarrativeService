...
=head2 log_open_narrative

  $log_result = $obj->log_open_narrative($params)
...
<pre>
$params is a NarrativeService.LogOpenParams
$log_result is a NarrativeService.boolean
LogOpenParams is a reference to a hash where the following keys are defined:
	context has a value which is a NarrativeService.LogContext
LogContext is a reference to a hash where the following keys are defined:
	narr_ref has a value which is a string
	narr_version has a value which is an int
	log_time has a value which is a NarrativeService.timestamp
	level has a value which is a string
timestamp is a string
boolean is an int
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function log_open_narrative (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to log_open_narrative:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'log_open_narrative');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "NarrativeService.log_open_narrative",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method log_open_narrative",
					    status_line => $self->{client}->status_line,
					    method_name => 'log_open_narrative',
				       );
    }
...
=head2 log_save_narrative

  $log_result = $obj->log_save_narrative($params)
...
<pre>
$params is a NarrativeService.LogSaveParams
$log_result is a NarrativeService.boolean
LogSaveParams is a reference to a hash where the following keys are defined:
	context has a value which is a NarrativeService.LogContext
LogContext is a reference to a hash where the following keys are defined:
	narr_ref has a value which is a string
	narr_version has a value which is an int
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function log_save_narrative (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to log_save_narrative:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'log_save_narrative');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "NarrativeService.log_save_narrative",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method log_save_narrative",
					    status_line => $self->{client}->status_line,
					    method_name => 'log_save_narrative',
				       );
    }
...
error: if an error occurred while looking up (found an unavailable copy, or the report is not accessible),
       this will have a sensible string. More or less.
