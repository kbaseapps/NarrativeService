...
SERVICE_CAPS = NarrativeService
SPEC_FILE = NarrativeService.spec
URL = https://kbase.us/services/narrativeservice
DIR = $(shell pwd)
LIB_DIR = lib
SCRIPTS_DIR = scripts
TEST_DIR = test
...
build-test-script:
	...
	echo 'python -m nose --with-coverage --cover-package=NarrativeService --cover-html --cover-html-dir=/kb/module/work/cover_html --nocapture .' >> $(TEST_DIR)/$(TEST_SCRIPT_NAME)
